...
public class NumericTotalsRepository {
    ...
    public static final String sumOfSalesSQL = "select sum(oper_sum) as sum_sales from wdfly.session_journal where cash_session_id=(select id from wdfly.cash_session where session_number=?) and oper_sum is not null and oper_type='30'";
    public static final String sumOfCashSQL = "select sum(cash_sum) as cash_sum from wdfly.session_journal inner join wdfly.ticket on session_journal.ticket_id=ticket.id where cash_session_id=(select id from wdfly.cash_session where session_number=?) and fiscal=1 and oper_type='30' and cash_sum is not null";
    public static final String sumOfNoncashSQL = "select sum(cashless_sum) as cashless_sum from wdfly.session_journal inner join wdfly.ticket on session_journal.ticket_id=ticket.id where cash_session_id=(select id from wdfly.cash_session where session_number=?) and fiscal=1 and oper_type='30' and cashless_sum is not null";
    ...
    public static final String salesValueSQL = "select sum(oper_sum) as sales_value from wdfly.session_journal where cash_session_id=(select id from wdfly.cash_session where session_number=?) and oper_type='30' and oper_status=30";
    public static final String numOfSalesSQL = "select count(*) as num_sales from wdfly.session_journal where cash_session_id=(select id from wdfly.cash_session where session_number=?) and oper_type='30'";
    ...
    private Number queriesExecutor(String statementSql) {
        Number data = 0;
    ...
    public Totals totalsBuilder() {
        return new Totals(
                ...
                queriesExecutor(numOfSalesSQL).longValue(),
                (long) (queriesExecutor(sumOfSalesSQL).doubleValue() * 100),
                (long) (queriesExecutor(sumOfNoncashSQL).doubleValue() * 100),
                (long) (queriesExecutor(sumOfCashSQL).doubleValue() * 100),
                queriesExecutor(numOfRefundsSQL).longValue(),
                (long) (queriesExecutor(sumOfRefundsSQL).doubleValue() * 100),
                queriesExecutor(numOfCashInSQL).longValue(),
                (long) (queriesExecutor(sumOfCashInSQL).doubleValue() * 100),
                queriesExecutor(numOfCashOutSQL).longValue(),
                (long) (queriesExecutor(sumOfCashOutSQL).doubleValue() * 100),
                0,
                0,
                queriesExecutor(numOfAnnulSQL).longValue(),
                (long) (queriesExecutor(sumOfAnnulSQL).doubleValue() * 100),
                0,
                0,
                (long) (queriesExecutor(salesValueSQL).doubleValue() * 100)
        );
    }
}